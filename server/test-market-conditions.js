...
const { TIME_CONDITIONS, VOLATILITY_CONDITIONS } = require('./services/analysis/marketCondition');
...
const testMarketConditions = async () => {
    try {
        ...
        // Test specifically for EA_LOW conditions (to match your templates)
        console.log('\nTesting specifically for Early Afternoon with Low Volatility:');
        const eaLowConditions = {
            time: TIME_CONDITIONS.EARLY_AFTERNOON,
            volatility: VOLATILITY_CONDITIONS.LOW
        };
        console.log('EA_LOW Conditions:', eaLowConditions);
        
        const eaLowMatches = await marketCondition.findMatchingTemplates(eaLowConditions);
        console.log('Matching ATM templates for EA_LOW:', eaLowMatches.atm.map(t => t.name));
        console.log('Matching Flazh templates for EA_LOW:', eaLowMatches.flazh.map(t => t.name));
